/**
 * WooCommerce product → the platform's normalized product.
 *
 * The interesting part is `attributes`. consuela hardcoded three cleaning-supply
 * meta keys (`ph_jelleg`, `felulet_lista`, `tiltott_feluletek`) into columns, a
 * schema and a SQL filter, which is why it could only ever serve one vertical.
 * Here every non-internal meta key and every product attribute is emitted
 * verbatim, and the shop admin decides on the platform what each one means —
 * whether it is embedded, whether it shows on the card, whether it filters
 * recommendations. A cleaning shop's `tiltott_feluletek` still arrives; it is
 * just no longer special to this code.
 */
import type {
  AdapterProduct,
...
import type { WooMeta, WooProduct } from './types'
...
export function toAttributes(
  ...
  for (const meta of product.meta_data ?? []) {
    if (isInternalMetaKey(meta.key)) continue
    const value = toAttributeValue(meta)
    if (value !== null) attributes[meta.key] = value
  }

  for (const attribute of product.attributes ?? []) {
    const key = slugify(attribute.name)
    if (key.length === 0 || key in attributes) continue
    const options = (attribute.options ?? [])
      .map((option) => stripHtml(String(option)))
      .filter((option) => option.length > 0)
    if (options.length > 0) attributes[key] = options
  }
 ...
 */
export function attributeHints(products: WooProduct[]): ProductAttributeHint[] {
  const kinds = new Map<string, 'text' | 'list' | 'number'>()
...
function toAttributeValue(meta: WooMeta): string | string[] | null {
  const { value } = meta
