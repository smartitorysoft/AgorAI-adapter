/**
 * Verifying who the shopper is, from a blob the theme signed.
 *
 * The storefront never sends a bare customer id — an unsigned id in a public
 * request body is a "show me someone else's order history" hole. The theme
 * snippet signs `${id}|${exp}` with Liquid's `hmac_sha256`, using a secret
 * shared with this adapter, and the whole of the check below is why that is
 * worth doing properly:
 *
 *  - expiry, with clock skew allowed, because Shopify's renderer and this
 *    container do not share a clock;
...
/** Longer than the snippet ever issues. Anything beyond it is a forgery. */
 ...
 * the compact `id.exp.sig` the snippet emits, and a raw JSON object for a
...
export function describeFailure(reason: IdentityFailure): string {
  switch (reason) {
    ...
    case 'expired': {
      return 'The identity token has expired — check clock drift between the storefront and this adapter.'
    }
    case 'implausible-lifetime': {
      return 'The identity token expires further out than the snippet ever issues.'
    }
    case 'bad-signature': {
      return 'Identity signature mismatch — download the theme snippet again; its secret must equal SHOPIFY_IDENTITY_SECRET on this project.'
    }
  }
}
