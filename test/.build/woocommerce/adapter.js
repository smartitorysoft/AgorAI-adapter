...
const adapter = (0, agorai_adapter_1.defineAdapter)({
    name: 'woocommerce',
    version: '1.0.0',
    ...
    async health(ctx) {
        const warnings = [];
        const { body: sample, headers } = await (0, client_1.wooGet)(ctx, 'products', { per_page: ATTRIBUTE_SAMPLE_SIZE, status: 'publish' });
        const currency = await (0, client_1.probeCurrency)(ctx);
        if (!currency.detected) {
            warnings.push(`Could not read the shop's currency, so prices are labelled ${currency.code}. ` +
                'Set the currency override on this screen if that is wrong.');
        }
        if ((0, config_1.setting)(ctx, 'WP_IDENTITY_SECRET').length === 0) {
            warnings.push('No identity secret is set, so every shopper is treated as a guest and ' +
            ...
            warnings.push('This API key cannot read orders. Order history will be unavailable — ' +
        ...
        if (!(0, config_1.booleanSetting)(ctx, 'WOOCOMMERCE_SYNC_VARIANTS') &&
            sample.some((product) => product.type === 'variable')) {
            warnings.push('This shop has variable products, and reading variations is off — the ' +
        ...
        return {
            ok: true,
            storeName: await (0, client_1.readStoreName)(ctx),
            productCount: (0, client_1.totalFrom)(headers),
            ...
            productAttributes: (0, product_1.attributeHints)(sample),
        };
    },
    catalog: {
        /**
        ...
        async list(ctx, { cursor, updatedSince, limit }) {
            ...
            if (updatedSince)
                query.modified_after = toWooDate(updatedSince);
            const { body, headers } = await (0, client_1.wooGet)(ctx, 'products', query);
            const currency = await (0, client_1.resolveCurrency)(ctx);
            const items = body.map((product) => (0, product_1.toProduct)(product, currency));
            if ((0, config_1.booleanSetting)(ctx, 'WOOCOMMERCE_SYNC_VARIANTS')) {
                await (0, variants_1.attachVariants)(ctx, items, currency);
            }
            return {
                items,
                ...
                nextCursor: page < (0, client_1.totalPagesFrom)(headers) ? String(page + 1) : null,
                total: (0, client_1.totalFrom)(headers),
            };
        },
        /**
        ...
        async get(ctx, ids) {
            ...
            if (numeric.length === 0)
                return [];
            const { body } = await (0, client_1.wooGet)(ctx, 'products', {
        ...
        async categories(ctx) {
            const collected = [];
            for (let page = 1;; page++) {
                const { body, headers } = await (0, client_1.wooGet)(ctx, 'products/categories', { page, per_page: MAX_PER_PAGE, orderby: 'id', order: 'asc' });
                collected.push(...body.map((category) => ({
                ...
                if (page >= (0, client_1.totalPagesFrom)(headers))
                    break;
            }
            return collected;
        },
    ...
    customer: {
        ...
        resolveIdentity(ctx, token) {
            const secret = (0, config_1.setting)(ctx, 'WP_IDENTITY_SECRET');
            if (secret.length === 0)
                return Promise.resolve(null);
            const result = (0, identity_1.verifyIdentity)(secret, token);
            if (!result.ok) {
                // Logged, not raised: this is diagnostics for the shop admin, and it
                // must never turn a logged-out visitor's chat into an error.
                console.warn(`[woocommerce] identity rejected: ${(0, identity_1.describeFailure)(result.reason)}`);
                return Promise.resolve(null);
            }
        ...
        async listOrders(ctx, customerId, limit) {
            ...
            if (!Number.isInteger(customer) || customer <= 0)
                return [];
            const configured = (0, config_1.numericSetting)(ctx, 'CUSTOMER_ORDERS_LIMIT', DEFAULT_ORDERS_LIMIT);
            const wanted = Math.max(0, Math.min(limit, configured));
            if (wanted === 0)
                return [];
            const { body } = await (0, client_1.wooGet)(ctx, 'orders', {
                customer,
...
// Helpers
// ---------------------------------------------------------------------------
async function productUrl(ctx, id) {
    ...
    if (!Number.isInteger(numeric) || numeric <= 0)
        return `/?p=${id}`;
    try {
        const { body } = await (0, client_1.wooGet)(ctx, `products/${numeric}`, {
            _fields: 'permalink',
        });
...
async function categoryUrl(ctx, id) {
    const numeric = Number.parseInt(id, 10);
    if (!Number.isInteger(numeric) || numeric <= 0) {
        return `/?product_cat=${encodeURIComponent(id)}`;
    }
    try {
        const { body } = await (0, client_1.wooGet)(ctx, `products/categories/${numeric}`, { _fields: 'slug' });
...
async function canReadOrders(ctx) {
    try {
        await (0, client_1.wooGet)(ctx, 'orders', { per_page: 1, _fields: 'number' }, ORDERS_TIMEOUT_MS);
        return true;
    }
    catch {
        return false;
